...
function handleButtonAction(e, actionFunction) {
    // Molto bene: blocca la propagazione al gameContainer.
    e.stopPropagation(); 
...
// 🆕 Listener per i pulsanti di Menu e Game Over con blocco della propagazione

// Listener per il pulsante START
startButton.addEventListener('click', (e) => {
    e.stopPropagation(); // Essenziale per bloccare l'evento prima che raggiunga il gameContainer
    startGame();
});

// Listener per i pulsanti RESTART
restartButton.addEventListener('click', (e) => {
    e.stopPropagation();
    resetGame();
});

// Listener per il pulsante BACK TO MENU
menuButton.addEventListener('click', (e) => {
    e.stopPropagation();
    backToMenu();
});

// Listener TOUCH END per il pulsante START
startButton.addEventListener('touchend', (e) => {
    e.preventDefault(); // Impedisce il default (doppio click)
    e.stopPropagation();
    startGame();
});

// Listener TOUCH END per il pulsante RESTART
...
// Listener TOUCH END per il pulsante BACK TO MENU
...
// 🛑 Listener per il salto del personaggio (tasto Spazio) - OK
...
/**
 * 💡 NUOVA LOGICA CENTRALIZZATA PER TOUCH/CLICK sul contenitore.
 * * Questa funzione gestisce sia i tocchi (touchstart) che i click,
 * assicurandosi che l'azione avvenga SOLO se l'elemento toccato NON è un pulsante.
 */
...
// Listener per il salto del personaggio (TOCCO su qualsiasi punto del contenitore di gioco - Mobile)
gameContainer.addEventListener('touchstart', (e) => {
    // 🛑 MODIFICA CHIAVE: Applica e.preventDefault() SOLO se l'elemento toccato NON è un pulsante.
    if (e.target.tagName !== 'BUTTON') { 
        e.preventDefault(); 
        
        // Esegui il salto solo se il gioco è in esecuzione e non è Game Over
        if (!isGameOver) {
            jump();
        }
    }
    
    // Se e.target.tagName È 'BUTTON', non facciamo nulla qui e lasciamo che l'evento
    // passi al pulsante e generi il click come da standard.
});
...
// 🆕 NUOVO: Listener per il salto del personaggio (TOCCO su qualsiasi punto del contenitore di gioco)
gameContainer.addEventListener('touchstart', (e) => {
    // Impedisce lo scorrimento predefinito (se necessario, ma il tuo CSS ha già overflow: hidden)
    e.preventDefault(); 
    
    // Controlla che il gioco sia in esecuzione (o non sia Game Over)
    if (!isGameOver) {
        jump();
    }
});

// 🆕 NUOVO: Listener per il salto del personaggio (CLICK del mouse su qualsiasi punto del contenitore)
...
// 🆕 NUOVO: Listener per il salto del personaggio (TOCCO su qualsiasi punto del contenitore di gioco)
gameContainer.addEventListener('touchstart', (e) => {
    // 🛑 Rimuovi la logica del e.preventDefault() qui
    
    // Controlla che il gioco sia in esecuzione (o non sia Game Over)
    if (!isGameOver && e.target.tagName !== 'BUTTON') { // Aggiungi il controllo sul tag qui
        jump();
    }
});

// 🆕 NUOVO: Listener per il salto del personaggio (CLICK del mouse su qualsiasi punto del contenitore)
gameContainer.addEventListener('click', (e) => { // Importante: prendi l'evento e
    if (!isGameOver && e.target.tagName !== 'BUTTON') { // Aggiungi il controllo sul tag qui
        jump();
    }
});
